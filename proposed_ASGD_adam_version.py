...
class Proposed_ASGD_adam_version(optimizer.Optimizer):
    ...
    def __init__(self, learning_rate_min=0.0001, learning_rate_max=0.01, adaptive_variable=0.5, beta1=0.9, beta2=0.99, epsilon=1e-8,
               use_locking=False, name="Proposed_ASGD_adam_version"):
        super(Proposed_ASGD_adam_version, self).__init__(use_locking, name)
    ...
    def _prepare(self):
        ...
        adaptive = self._call_if_callable(self._adaptive_t)
       ...
       # self._adaptive_t = ops.convert_to_tensor(adaptive, name="adaptive")
    ...
    def _apply_dense(self, grad, var):
       ...
       # vhat_t = state_ops.assign(vhat, math_ops.maximum(v_t, vhat))
        #vhat_t = state_ops.assign(vhat, math_ops.maximum(v_t, vhat))
        v_p = math_ops.sqrt(v_t)
        g_min_mean = ((tf.math.sign((tf.compat.v1.math.reduce_mean((v_t))) - (v_t)) + 1) /2)  
        g_max_mean =  ((tf.math.sign((v_t) - (tf.compat.v1.math.reduce_mean((v_t)))) + 1)/ 2)
          
            
       # float ( tf.math.count_nonzero(g_max)) <(float(tf.math.count_nonzero(g_min_mean))) and
       # @tf.function
        var_update=tf.cond(tf.math.less( ( ( tf.math.count_nonzero(g_max_mean))), tf.cast(adaptive, tf.int64) *  ((tf.math.count_nonzero(g_min_mean)))), lambda:  state_ops.assign_sub(var,
